"""
training/train_final_model_with_all_curves.py

Train the repo's current final model setup:
  - architecture: models.cnn_model.EnhancementCNN
  - loss: hybrid loss (L1 + Edge + Color)
...
Outputs are written to dedicated folders:
  - results/final_model_training_curves/
  - logs/final_model_training_curves/

Usage:
    python training/train_final_model_with_all_curves.py
"""
...
RESULTS_SUBDIR = os.path.join(RESULTS_DIR, "final_model_training_curves")
LOGS_SUBDIR = os.path.join(LOGS_DIR, "final_model_training_curves")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train final model and save loss, PSNR, and SSIM epoch curves"
    )
...
def main():
    ...
    master_log_path = os.path.join(LOGS_SUBDIR, "final_model_epoch_metrics.csv")
    ...
    save_single_curve(
        os.path.join(RESULTS_SUBDIR, "final_model_train_loss_vs_epoch.png"),
        epochs_x,
        train_losses,
        "Training Loss",
        "Final Model Training Loss vs Epoch",
        "steelblue",
    )
    save_single_curve(
        os.path.join(RESULTS_SUBDIR, "final_model_val_loss_vs_epoch.png"),
        epochs_x,
        val_losses,
        "Validation Loss",
        "Final Model Validation Loss vs Epoch",
        "darkorange",
    )
    save_single_curve(
        os.path.join(RESULTS_SUBDIR, "final_model_psnr_vs_epoch.png"),
        epochs_x,
        val_psnrs,
        "PSNR",
        "Final Model PSNR vs Epoch",
        "royalblue",
    )
    save_single_curve(
        os.path.join(RESULTS_SUBDIR, "final_model_ssim_vs_epoch.png"),
        epochs_x,
        val_ssims,
        "SSIM",
        "Final Model SSIM vs Epoch",
        "seagreen",
    )

    save_metric_csv(
        os.path.join(LOGS_SUBDIR, "final_model_train_loss_log.csv"),
        epochs_x,
        train_losses,
        "train_loss",
    )
    save_metric_csv(
        os.path.join(LOGS_SUBDIR, "final_model_val_loss_log.csv"),
        epochs_x,
        val_losses,
        "val_loss",
    )
    save_metric_csv(
        os.path.join(LOGS_SUBDIR, "final_model_psnr_log.csv"),
        epochs_x,
        val_psnrs,
        "val_psnr",
    )
    save_metric_csv(
        os.path.join(LOGS_SUBDIR, "final_model_ssim_log.csv"),
        epochs_x,
        val_ssims,
        "val_ssim",
    )
