"""
training/train_model1_with_all_curves.py

Train Model 1 on a paired train/validation split from EUVP trainA/trainB and save:
...
Outputs are written to dedicated folders:
  - results/model1_training_curves/
  - logs/model1_training_curves/

Usage:
    python training/train_model1_with_all_curves.py
"""
...
from models.model1 import EnhancementCNN


RESULTS_SUBDIR = os.path.join(RESULTS_DIR, "model1_training_curves")
LOGS_SUBDIR = os.path.join(LOGS_DIR, "model1_training_curves")
CHECKPOINT_SUBDIR = os.path.join(CHECKPOINTS_DIR, "model1_training_curves")


def parse_args():
    parser = argparse.ArgumentParser(
        description="Train Model 1 and save loss, PSNR, and SSIM epoch curves"
    )
...
def main():
    ...
    master_log_path = os.path.join(LOGS_SUBDIR, "model1_epoch_metrics.csv")
    ...
    save_single_curve(
        os.path.join(RESULTS_SUBDIR, "model1_train_loss_vs_epoch.png"),
        epochs_x,
        train_losses,
        "Training Loss",
        "Model 1 Training Loss vs Epoch",
        "steelblue",
    )
    save_single_curve(
        os.path.join(RESULTS_SUBDIR, "model1_val_loss_vs_epoch.png"),
        epochs_x,
        val_losses,
        "Validation Loss",
        "Model 1 Validation Loss vs Epoch",
        "darkorange",
    )
    save_single_curve(
        os.path.join(RESULTS_SUBDIR, "model1_psnr_vs_epoch.png"),
        epochs_x,
        val_psnrs,
        "PSNR",
        "Model 1 PSNR vs Epoch",
        "royalblue",
    )
    save_single_curve(
        os.path.join(RESULTS_SUBDIR, "model1_ssim_vs_epoch.png"),
        epochs_x,
        val_ssims,
        "SSIM",
        "Model 1 SSIM vs Epoch",
        "seagreen",
    )

    save_metric_csv(
        os.path.join(LOGS_SUBDIR, "model1_train_loss_log.csv"),
        epochs_x,
        train_losses,
        "train_loss",
    )
    save_metric_csv(
        os.path.join(LOGS_SUBDIR, "model1_val_loss_log.csv"),
        epochs_x,
        val_losses,
        "val_loss",
    )
    save_metric_csv(
        os.path.join(LOGS_SUBDIR, "model1_psnr_log.csv"),
        epochs_x,
        val_psnrs,
        "val_psnr",
    )
    save_metric_csv(
        os.path.join(LOGS_SUBDIR, "model1_ssim_log.csv"),
        epochs_x,
        val_ssims,
        "val_ssim",
    )

    final_checkpoint_path = os.path.join(CHECKPOINT_SUBDIR, "model1_final.pth")
