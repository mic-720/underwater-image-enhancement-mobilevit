...
Final Model 3:
CNN encoder-decoder with MobileViT-style bottleneck, skip connections,
and residual output.
"""
...
from config import MOBILEVIT_DEPTH, MOBILEVIT_DIM, MOBILEVIT_HEADS
...
class MobileViTBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        identity = x
        x = self.local_conv(x)
        x_flat = x.reshape(batch_size, channels, height * width).permute(0, 2, 1)
        x_flat = self.transformer(x_flat)
        x = x_flat.permute(0, 2, 1).reshape(batch_size, channels, height, width)
        x = self.proj_conv(x)
        return x + identity
